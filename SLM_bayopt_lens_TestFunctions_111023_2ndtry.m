...
Zern_amplitudes_2 = optimizableVariable("Zernikamplitudes_astig1",[0.10,0.25],"Type","real");
Zern_amplitudes_3 = optimizableVariable("Zernikamplitudes_astig2",[0.05,0.15],"Type","real");
...
foc_max_count_h = @(x)foc_peak_intensity(x);
results = bayesopt(foc_max_count_h,Zern_amplitudes_1);
opt_foc = results.XAtMinObjective.Zernikamplitudes_foc;

astig_max_count_h = @(x)astig_peak_intensity(x,opt_foc);
results1 = bayesopt(astig_max_count_h,[Zern_amplitudes_2,Zern_amplitudes_3]);
opt_astig1 = results1.XAtMinObjective.Zernikamplitudes_astig1;
opt_astig2 = results1.XAtMinObjective.Zernikamplitudes_astig2;

comma_max_count_h = @(x)comma_peak_intensity(x,opt_foc,opt_astig1,opt_astig2);
results2 = bayesopt(comma_max_count_h,[Zern_amplitudes_4,Zern_amplitudes_5]);
...
final_max_count_h = @(x)final_peak_intensity(x);
...
function foc_max_count = foc_peak_intensity(xx)%TAKE PICTURE
    ...
    lens = T;
    %zernike corrections
    pist = 0; x_tilt = 0; y_tilt = 0; h_coma = 0; v_coma = 0; spher = 0;
    
    HoloL = lens + zernike(pist, x_tilt , y_tilt , xx.Zernikamplitudes_foc , 0, 0, 0 ,0, spher, H,V,RHO,PHI);
...
%%
%% Objective function for astig
function astig_max_count = astig_peak_intensity(xx, opt_foc)%TAKE PICTURE
    %% SLM and laser parameters
    H=1280; V=1024; %%Number of Horizontal and Vertical pixels
    PixelSize = 12.5e-3; % [mm] pixel
    x = PixelSize.*(-H/2:(H/2-1)); %[mm] x width of window
    ...
    HoloL = lens + zernike(pist, x_tilt , y_tilt , opt_foc , xx.Zernikamplitudes_astig1, xx.Zernikamplitudes_astig2, 0 ,0, spher, H,V,RHO,PHI);
...
function comma_max_count = comma_peak_intensity(xx, opt_foc,opt_astig1,opt_astig2)%TAKE PICTURE
    %% SLM and laser parameters
    H=1280; V=1024; %%Number of Horizontal and Vertical pixels
    PixelSize = 12.5e-3; % [mm] pixel
    x = PixelSize.*(-H/2:(H/2-1)); %[mm] x width of window
    ...
    HoloL = lens + zernike(pist, x_tilt , y_tilt , opt_foc , opt_astig1, opt_astig2, xx.Zernikamplitudes_comma1 , xx.Zernikamplitudes_comma2, spher, H,V,RHO,PHI);
...
%% Objective function 5d
function final_max_count = final_peak_intensity(xx)%TAKE PICTURE
    %% SLM and laser parameters
    H=1280; V=1024; %%Number of Horizontal and Vertical pixels
    PixelSize = 12.5e-3; % [mm] pixel
    x = PixelSize.*(-H/2:(H/2-1)); %[mm] x width of window
