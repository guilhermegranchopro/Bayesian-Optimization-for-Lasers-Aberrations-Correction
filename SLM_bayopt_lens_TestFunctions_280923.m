...
Zern_amplitudes_2 = optimizableVariable("Zernikamplitudes_astig1",[0,0.25],"Type","real");

max_count_h = @(x)peak_intensity(x,cam);
results = bayesopt(max_count_h,Zern_amplitudes_1)
...
function max_count = peak_intensity(xx,cam)%TAKE PICTURE
    ...
    HoloL = lens + zernike(pist, x_tilt , y_tilt , xx.Zernikamplitudes_foc , 0.19431 ,  0 , h_coma , v_coma , spher, H,V,RHO,PHI);
    ...
    %% CAMERA part
        ...
        maxPixelValue = double(2^cam.BitDepth - 3);
        if (max_imageData2D >= maxPixelValue) && 0
            % Release the camera
            disp('Releasing the camera');
            cam.Dispose;
            delete(cam);
